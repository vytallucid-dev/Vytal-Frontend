...
> = {
  success: { label: "Success", color: "text-success", icon: Icons.success },
  error: { label: "Error", color: "text-destructive", icon: Icons.xCircle },
  market_closed: {
    label: "Market Closed",
    color: "text-warning",
...
const StockPricesPage = () => {
  // Trigger is now JOB-BASED (enqueue → poll), mirroring the backfill below —
  // the ingest fans a full-universe snapshot recompute that can't run inline.
  const [triggerStatus, setTriggerStatus] = useState<JobStatus>("idle");
  const [triggerError, setTriggerError] = useState<string | null>(null);
  const [triggerJobId, setTriggerJobId] = useState<string | null>(null);
  const [triggerJob, setTriggerJob] = useState<JobData | null>(null);
  ...
  const startPolling = useCallback(
    (id: string) => {
      stopPolling();
      pollRef.current = setInterval(() => pollTriggerJob(id), POLL_INTERVAL_MS);
      pollTriggerJob(id);
    },
    [pollTriggerJob, stopPolling],
  );
  ...
  const handleTrigger = async () => {
    ...
    try {
      const res = await adminFetch(`${API_BASE}/admin/prices/trigger`, {
        method: "POST",
      ...
      if (!json.success || !json.data) {
        setTriggerError(json.error ?? "Trigger request failed.");
        setTriggerStatus("error");
        toast.error("Price trigger failed", {
          description: json.error ?? "The server rejected the request.",
        });
        return;
      }
      setTriggerJobId(json.data.jobId);
      setTriggerStatus("polling");
      startPolling(json.data.jobId);
      toast.success("Ingest enqueued", {
        description: "Running in the background — tracking progress below.",
      });
    } catch {
      setTriggerError("Network error — server unreachable.");
      setTriggerStatus("error");
      toast.error("Network error", {
        description: "Server unreachable — nothing was enqueued.",
      });
    }
  };
  ...
  const isTriggering =
    triggerStatus === "submitting" || triggerStatus === "polling";
      ...
      <div className="w-full border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm py-8 px-6 shadow-lg shadow-ring/10">
        ...
        <div className="flex items-center gap-3 mb-1">
          <div className="p-2 rounded-lg bg-primary/10 border border-primary/20">
            <Icons.trendUp className="size-5 text-primary" weight="duotone" />
          </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              {(triggerStatus === "done" || triggerStatus === "error") && (
            ...
            <p className="text-[11px] text-muted-foreground/50 font-mono">
              POST {API_BASE}/admin/prices/trigger — runs as a background job
            </p>
          </div>
          ...
          <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-5 min-h-[160px]">
            ...
            {triggerJob && <BackfillJobPanel job={triggerJob} />}

            {triggerJobId && !triggerJob && (
...
const BackfillSection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await adminFetch(`${API_BASE}/admin/prices/backfill`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
const BackfillJobPanel = ({ job }: { job: JobData }) => {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-xl border border-border/50 bg-muted/20 p-4">
      ...
      {job.result && (() => {
        // EOD_PRICES_DAILY returns an ARRAY (one IngestPricesResult per re-checked
        // trading day); backfill / refetch return a single object. Normalize both.
        const raw = job.result as unknown;
        const r: Record<string, number> = Array.isArray(raw)
          ? (raw as Array<Record<string, number>>).reduce(
              (acc, d) => ({
                totalFetched: (acc.totalFetched ?? 0) + (d?.totalFetched ?? 0),
                totalInserted: (acc.totalInserted ?? 0) + (d?.totalInserted ?? 0),
                totalSkipped: (acc.totalSkipped ?? 0) + (d?.totalSkipped ?? 0),
              }),
  ...
  return (
    <div className="flex flex-col gap-2 p-3.5 rounded-lg border border-border/50 bg-muted/20 hover:bg-muted/40 transition-colors duration-150">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <StatusIcon className={`size-4 shrink-0 ${meta.color}`} />
