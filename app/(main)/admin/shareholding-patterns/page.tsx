      ...
      <div className="w-full border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm py-8 px-6 shadow-lg shadow-ring/10">
        ...
        <div className="flex items-center gap-3 mb-1">
          <div className="p-2 rounded-lg bg-danger/10 border border-danger/20">
            <Icons.sector className="size-5 text-danger" weight="duotone" />
          </div>
        ...
        <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-4">
          ...
          {logsLoading && logs.length === 0 && (
            <div className="flex flex-col items-center justify-center gap-3 py-12">
              <Icons.spinner className="size-6 text-danger animate-spin" />
    ...
    <div className="flex flex-col gap-2 p-3.5 rounded-lg border border-border/50 bg-muted/20 hover:bg-muted/40 transition-colors duration-150">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span
            className={`text-xs font-bold uppercase tracking-wide ${isSuccess ? "text-success" : "text-destructive"}`}
          >
            {log.stockSymbol}
          </span>
    ...
    <div className="flex flex-col gap-3 rounded-xl border border-border/50 bg-muted/20 p-4">
      ...
      {!isTerminal && (
        <div className="w-full h-1.5 rounded-full bg-border/50 overflow-hidden">
          <div className="h-full rounded-full bg-danger transition-all duration-500" style={{ width: `${job.progress}%` }} />
        </div>
      )}
      {job.progressNote && (
  ...
  const submit = async (url: string, body: object) => {
    ...
    try {
      ...
      if (!json.success || !json.data) {
        setJobError(json.error ?? "Request failed."); setJobStatus("error");
        toast.error("Shareholding job failed", {
          description: json.error ?? "The server rejected the request.",
        });
        return;
      }
      setJobId(json.data.jobId); setJobStatus("polling"); startPolling(json.data.jobId);
      toast.success("Shareholding job started", { description: "Tracking the job's progress below." });
    } catch {
      setJobError("Network error — server unreachable."); setJobStatus("error");
      toast.error("Network error", { description: "Server unreachable — the job wasn't started." });
    }
  };
    ...
    <>
      <div className="flex items-center gap-2.5 flex-wrap">
        <Button
          ...
          disabled={isRunning}
          size="sm"
          className="gap-2 bg-danger hover:bg-danger active:bg-danger text-foreground font-semibold px-5 py-2.5 rounded-lg shadow-md shadow-danger/30 hover:shadow-lg hover:shadow-danger/40 transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {isRunning ? (
            <><Icons.spinner className="size-4 animate-spin" />Running…</>
          ) : (
            <><Icons.play className="size-4" />{label}</>
          )}
        </Button>
        {canCancel && (
          <Button
    ...
    <form onSubmit={handleSubmit} className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-5">
      ...
      <div className="flex flex-wrap gap-2">
        {["Fetches quarterly holding data", "Skips existing quarters", "Safe to run multiple times"].map((label) => (
          <span key={label} className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-danger/10 border border-danger/20 text-danger">
            {label}
          </span>
        ))}
      </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-5">
      <div className="flex items-start gap-3">
        <div className="p-2 rounded-lg bg-danger/10 border border-danger/20 shrink-0">
          <Icons.bolt className="size-4 text-danger" />
        </div>
        <div>
...
const BackfillCard = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-5">
      <div className="flex items-start gap-3">
        <div className="p-2 rounded-lg bg-danger/10 border border-danger/20 shrink-0">
          <Icons.database className="size-4 text-danger" />
        </div>
        <div>
