...
const TRIGGER_ACTIONS: TriggerAction[] = [
  {
    ...
    btnClass:
      "bg-primary hover:bg-primary/90 active:bg-primary/80 shadow-primary/30 hover:shadow-primary/40",
  ...
  },
  {
    ...
    btnClass:
      "bg-warning hover:bg-warning/90 active:bg-warning/80 shadow-warning/30 hover:shadow-warning/40",
  ...
  },
  {
    ...
    btnClass:
      "bg-success hover:bg-success/90 active:bg-success/80 shadow-success/30 hover:shadow-success/40",
      ...
      <div className="w-full border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm py-8 px-6 shadow-lg shadow-ring/10">
        ...
        <div className="flex items-center gap-3 mb-1">
          <div className="p-2 rounded-lg bg-primary/10 border border-primary/20">
            <Icons.news className="size-5 text-primary" weight="duotone" />
          </div>
...
const TriggerCard = ({ action }: { action: TriggerAction }) => {
  ...
  const handleRun = async () => {
    ...
    setCancelError(null);
    try {
      const res = await adminFetch(`${API_BASE}${action.endpoint}`, {
        method: "POST",
    ...
    try {
      ...
      if (!json.success) {
        ...
        toast.success("Cancellation requested", {
          description: "The job is being stopped.",
        });
      }
    } catch {
      ...
      <div className="flex items-center gap-2.5">
        {jobStatus === "done" || jobStatus === "error" ? (
          ...
          <Button
            ...
            disabled={isRunning}
            size="sm"
            className={`gap-2 text-foreground font-semibold shadow-md transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed ${action.btnClass}`}
          >
            {isRunning ? (
              <><Icons.spinner className="size-3.5 animate-spin" />{jobStatus === "submitting" ? "Submitting…" : "Processing…"}</>
            ) : (
              <><Icons.play className="size-3.5" />Run</>
            )}
          </Button>
        )}
