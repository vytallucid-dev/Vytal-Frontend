...
> = {
  ...
  no_data: { label: "No Data", color: "text-warning", icon: Icons.clock },
...
const InsiderTradesPage = () => {
  ...
  const handleTrigger = async () => {
    ...
    try {
      const res = await adminFetch(`${API_BASE}/admin/insider-trades/trigger`, {
        method: "POST",
      ...
      if (json.success) {
        toast.success("Insider trades ingested", {
          description:
            json.message ??
            "Latest disclosures fetched and new trades inserted.",
        });
        fetchLogs(1);
      } else {
        toast.error("Insider trades ingest failed", {
          description:
      ...
      <div className="w-full border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm py-8 px-6 shadow-lg shadow-ring/10">
        ...
        <div className="flex items-center gap-3 mb-1">
          <div className="p-2 rounded-lg bg-primary/10 border border-primary/20">
            <Icons.userFocus className="size-5 text-primary" weight="duotone" />
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {status === "loading" ? (
                  ...
                  <>
                    <Icons.play className="size-4" weight="fill" />
...
const BackfillSection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await adminFetch(`${API_BASE}/admin/insider-trades/backfill`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!json.success || !json.data) {
        ...
        setJobStatus("error");
        toast.error("Insider trades backfill couldn't start", {
          description: json.error ?? "Backfill request failed.",
        });
        return;
      }
      ...
      toast.success("Insider trades backfill started", {
        description: json.data.message ?? "Running as a background job.",
      });
    } catch {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      if (!json.success) {
        ...
        toast.error("Couldn't cancel the backfill", {
          description: json.error ?? "Failed to cancel job.",
        });
      } else {
      ...
      toast.error("Couldn't cancel the backfill", {
        description: "Network error — could not cancel job.",
      });
    } finally {
