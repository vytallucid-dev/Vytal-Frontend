...
const IndexPricesPage = () => {
  ...
  const fetchLogs = useCallback(async (page = 1) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/indices/index-logs?page=${page}&limit=10`,
      );
  ...
  const handleTrigger = async () => {
    ...
    try {
      const res = await adminFetch(`${API_BASE}/admin/indices/trigger`, {
        method: "POST",
      ...
      if (json.success) {
        fetchLogs(1);
        toast.success("Index price refresh triggered", {
          description: "The index price ingestion pipeline ran.",
        });
      } else {
        toast.error("Index refresh failed", {
      ...
      <div className="w-full border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm py-8 px-6 shadow-lg shadow-ring/10">
        ...
        <div className="flex items-center gap-3 mb-1">
          <div className="p-2 rounded-lg bg-primary/10 border border-primary/20">
            <Icons.chartLine className="size-5 text-primary" weight="duotone" />
          </div>
          <h1 className="text-2xl md:text-3xl font-bold">Index Prices</h1>
        </div>
        <p className="text-sm text-muted-foreground mt-2 ml-[3.25rem]">
          Manually trigger the daily ingestion of the NSE index archive
          (ind_close_all) — per-index OHLC plus P/E, P/B and dividend yield.
          Display-only data; it does not affect Health Scores.
        </p>
      </div>
          ...
          <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-6">
            <div>
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                Calling this trigger will fetch today&apos;s NSE index archive
                (ind_close_all), parse each index&apos;s end-of-day OHLC and
                valuation, and upsert them into the database on (index, date).
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {[
                "Fetches NSE index archive",
                "Upserts on (index, date)",
            ...
            <div className="flex items-center gap-3">
              <Button
                ...
                size="lg"
                className="gap-2.5 bg-primary hover:bg-primary active:bg-primary text-foreground font-semibold px-6 py-3 rounded-lg shadow-md shadow-primary/30 hover:shadow-lg hover:shadow-primary/40 transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {status === "loading" ? (
                  <>
            ...
            <p className="text-[11px] text-muted-foreground/50 font-mono">
              POST {API_BASE}/admin/indices/trigger
            </p>
          </div>
            ...
            {status === "loading" && (
              <div className="flex-1 flex flex-col items-center justify-center gap-3 py-6">
                ...
                <p className="text-sm text-muted-foreground">
                  Fetching and processing index values…
                </p>
              </div>
            )}
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <StatCard
                    icon={Icons.calendar}
                    label="Index Date"
                    value={formatDate(data.indexDate)}
                    color="text-primary"
...
const BackfillSection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await adminFetch(`${API_BASE}/admin/indices/backfill`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="w-full border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-6">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Fetch NSE index records for the past N days and upsert them into the database as a background job.
          ...
          <div className="flex items-center gap-2">
            {jobStatus === "done" || jobStatus === "error" ? (
              ...
              <Button
                type="submit"
                disabled={isRunning}
                className="gap-2 bg-primary hover:bg-primary text-foreground disabled:opacity-60"
              >
                {isRunning ? (
  ...
  return (
    <div className="flex flex-col gap-2 p-3.5 rounded-lg border border-border/50 bg-muted/20 hover:bg-muted/40 transition-colors duration-150">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <LogPill label="Date" value={log.indexDate} />
