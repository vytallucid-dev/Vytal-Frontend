...
function staleMeta(row: StatusRow): StaleMeta {
  if (row.hasCurrentQuarter) {
    return { label: "Current", cls: "bg-success/10 border-success/20 text-success" };
  }
...
export default function CasaAdminPage() {
  ...
  const loadStatus = useCallback(async () => {
    ...
    setTableError(null);
    try {
      const res = await adminFetch(`${API_BASE}/admin/bank-supplementary/casa/status`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await adminFetch(`${API_BASE}/admin/bank-supplementary/casa`, {
        method: "POST",
      ...
      if (!res.ok || !json.success) {
        ...
        toast.error(`CASA rejected for ${symbol} ${fiscalYear}/${quarter}`, { description: reason });
        return;
      }
      ...
      toast.success(`CASA ${action} — ${symbol} ${fiscalYear}/${quarter} = ${valueNum}%`, {
        description: (json.data?.note ?? "The value now drives this bank's F7 CASA score.") + warn,
      });
      setValue("");
      setSourceCitation("");
      setNotes("");
      ...
      toast.error("Network error", { description: "Could not reach the backend — nothing was saved." });
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="mx-auto flex w-full max-w-7xl min-w-0 flex-col gap-6">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-primary/12 text-primary ring-1 ring-primary/20">
              <Icons.scales className="size-5" weight="duotone" />
            </span>
            <div>
          ...
          <Button variant="outline" size="sm" onClick={loadStatus} disabled={tableStatus === "loading"} className="gap-2 shrink-0">
            <Icons.refresh className={`size-3.5 ${tableStatus === "loading" ? "animate-spin" : ""}`} />
            Refresh
          </Button>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          {summary && (
            <div className="hidden sm:flex items-center gap-2 text-[11px]">
              ...
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-warning/10 border border-warning/20 text-warning">
      ...
      <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-5">
        ...
        {notice && (
          ...
          >
            {notice.kind === "success" ? (
              <Icons.success className="size-5 text-success shrink-0 mt-0.5" />
            ) : (
              <Icons.warning className="size-5 text-destructive shrink-0 mt-0.5" />
            )}
            <div className="min-w-0">
              <p className={`text-sm font-medium ${notice.kind === "success" ? "text-success" : "text-destructive"}`}>
                {notice.title}
              </p>
              <p className={`text-xs mt-0.5 ${notice.kind === "success" ? "text-success/80" : "text-destructive/80"}`}>
                {notice.body}
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          {isPast && (
            <div className="flex items-start gap-2.5 p-3.5 rounded-lg bg-warning/10 border border-warning/30">
              <Icons.warningTriangle className="size-5 text-warning shrink-0 mt-0.5" />
              <div>
          ...
          {isFuture && (
            <div className="flex items-start gap-2.5 p-3 rounded-lg bg-info/10 border border-info/20">
              <Icons.warning className="size-4 text-info shrink-0 mt-0.5" />
          ...
          {isCurrent && (
            <p className="text-[11px] text-muted-foreground/70 flex items-center gap-1.5">
              <Icons.success className="size-3.5 text-success/70" />
          ...
          <p className="text-[11px] text-muted-foreground/50 font-mono flex items-center gap-1.5">
            <Icons.database className="size-3" />
