...
const PeerGroupMetricsPage = () => {
  ...
  const handleRunAll = async () => {
    ...
    try {
      ...
      if (!json.success || !json.data) {
        ...
        toast.error("Peer-group metrics run failed", {
      ...
      toast.success("Peer-group metrics run started", {
        description: "Recomputing all groups — tracking progress below.",
      });
    } catch {
      setAllJobError("Network error — server unreachable.");
      setAllJobStatus("error");
      toast.error("Network error", {
        description: "Server unreachable — the run wasn't started.",
      });
    }
  };
  ...
  const handleCancelAll = async () => {
    ...
    setAllCancelError(null);
    try {
      const res = await adminFetch(`${API_BASE}/admin/jobs/${allJobId}/cancel`, { method: "POST" });
      const json = await res.json();
      if (!json.success) {
        setAllCancelError(json.error ?? "Failed to cancel job.");
        toast.error("Couldn't cancel the run", {
          description: json.error ?? "The server rejected the request.",
        });
      } else {
        toast.success("Cancellation requested", { description: "The run is being stopped." });
      }
    } catch {
  ...
  const triggerFetch = async (
    ...
    try {
      ...
      if (json.success) {
        fetchLogs(1);
        toast.success("Peer-group metrics refreshed", {
          description: json.message ?? "The metrics recomputed successfully.",
        });
      } else {
        toast.error("Metrics refresh failed", {
          description: json.error ?? json.message ?? "The server rejected the request.",
        });
      }
    } catch {
      updateState(key, {
      ...
      toast.error("Network error", {
        description: "Server unreachable — nothing was recomputed.",
      });
    }
  };
      ...
      <div className="w-full border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm py-8 px-6 shadow-lg shadow-ring/10">
        ...
        <div className="flex items-center gap-3 mb-1">
          <div className="p-2 rounded-lg bg-primary/10 border border-primary/20">
            <Icons.chartBar className="size-5 text-primary" weight="duotone" />
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-5 flex flex-col gap-4">
            <div className="flex items-start gap-3">
              <div className="p-2 rounded-lg bg-primary/10 border border-primary/20 shrink-0">
                <Icons.globe className="size-4 text-primary" weight="duotone" />
              </div>
            ...
            <div className="flex items-center gap-2.5">
              {allJobStatus === "done" || allJobStatus === "error" ? (
                ...
                <Button
                  ...
                  size="sm"
                  className="gap-2 text-foreground font-semibold shadow-md transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed bg-primary hover:bg-primary active:bg-primary shadow-primary/30 hover:shadow-primary/40"
                >
                  {isAllRunning ? (
                    <><Icons.spinner className="size-3.5 animate-spin" />{allJobStatus === "submitting" ? "Submitting…" : "Processing…"}</>
                  ) : (
                    <><Icons.play className="size-3.5" />Run Full Ingest</>
                  )}
                </Button>
              )}
              ...
              {canCancelAll && (
                ...
                >
                  {allCancelLoading ? <Icons.spinner className="size-3.5 animate-spin" /> : <Icons.prohibit className="size-3.5" />}
                  Cancel
                </Button>
              )}
          ...
          <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-5 flex flex-col gap-4">
            <div className="flex items-start gap-3">
              <div className="p-2 rounded-lg bg-primary/10 border border-primary/20 shrink-0">
                <Icons.building className="size-4 text-primary" weight="duotone" />
              </div>
              <div className="min-w-0">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                className="w-full h-9 px-3 rounded-lg border border-border/60 bg-background/60 text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/40 transition-all"
              />
            </div>
          ...
          <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-5 flex flex-col gap-4">
            <div className="flex items-start gap-3">
              <div className="p-2 rounded-lg bg-warning/10 border border-warning/20 shrink-0">
                <Icons.hash className="size-4 text-warning" weight="duotone" />
              </div>
              <div className="min-w-0">
                <h2 className="font-semibold text-sm">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                className="w-full h-9 px-3 rounded-lg border border-border/60 bg-background/60 text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:ring-2 focus:ring-warning/40 focus:border-warning/40 transition-all"
              />
            </div>
            ...
            <div className="flex items-center gap-2.5">
              <Button
                ...
                size="sm"
                className="gap-2 text-foreground font-semibold shadow-md transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed bg-warning hover:bg-warning active:bg-warning shadow-warning/30 hover:shadow-warning/40"
              >
