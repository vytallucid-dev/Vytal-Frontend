...
const CorporateEventsPage = () => {
  ...
  const [logsMeta, setLogsMeta] = useState<{ total: number; totalPages: number; hasNext: boolean; hasPrev: boolean } | null>(null);
  ...
  const fetchLogs = useCallback(async (page = 1) => {
    ...
    setLogsError(null);
    try {
      const res = await fetch(`${API_BASE}/events/event-logs?page=${page}&limit=10`);
  ...
  const handleTrigger = async () => {
    ...
    try {
      const res = await adminFetch(`${API_BASE}/admin/events/refresh`, {
        method: "POST",
      ...
      if (json.success) {
        const d = json.data;
        toast.success("Corporate events imported", {
          description: d
            ? `${d.totalInserted.toLocaleString()} inserted · ${d.totalUpdated.toLocaleString()} updated · ${d.totalSkipped.toLocaleString()} skipped`
            : "Upcoming corporate events refreshed.",
        });
        fetchLogs(1);
      } else {
        toast.error("Corporate events ingest failed", {
          description:
            json.error ?? json.message ?? "The ingestion did not complete.",
        });
      }
    } catch {
      setResult({
      ...
      setStatus("error");
      toast.error("Couldn't reach the backend — nothing was saved.", {
        description: "Network error — server unreachable.",
      });
    }
  };
      ...
      <div className="w-full border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm py-8 px-6 shadow-lg shadow-ring/10">
        ...
        <div className="flex items-center gap-3 mb-1">
          <div className="p-2 rounded-lg bg-warning/10 border border-warning/20">
            <Icons.calendar weight="duotone" className="size-5 text-warning" />
          </div>
          <h1 className="text-2xl md:text-3xl font-bold">Corporate Events</h1>
        </div>
        <p className="text-sm text-muted-foreground mt-2 ml-[3.25rem]">
          Manually trigger the ingestion of upcoming corporate events — dividends,
          splits, bonuses, and rights issues — from the NSE feed.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-6">
            <div>
              <h2 className="font-semibold text-base mb-1">Event Ingest Trigger</h2>
              <p className="text-xs text-muted-foreground leading-relaxed">
                Calling this trigger will fetch the next 30 days of corporate
                event data from NSE, deduplicate existing records, and upsert
                any new or updated events into the database.
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {[
                "Fetches NSE corporate events",
                "Upserts changed records",
                "Safe to run multiple times",
              ].map((label) => (
                <span
                  key={label}
                  className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-warning/10 border border-warning/20 text-warning"
                >
                  {label}
                </span>
              ))}
            ...
            <div className="flex items-center gap-3">
              <Button
                ...
                size="lg"
                className="gap-2.5 bg-warning hover:bg-warning active:bg-warning text-foreground font-semibold px-6 py-3 rounded-lg shadow-md shadow-warning/30 hover:shadow-lg hover:shadow-warning/40 transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {status === "loading" ? (
                  <>
                  ...
                  <>
                    ...
                    Trigger Corporate Events Ingest
                  </>
                )}
              </Button>
            ...
            <p className="text-[11px] text-muted-foreground/50 font-mono">
              POST {API_BASE}/admin/events/refresh
            </p>
          </div>
          ...
          <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-5 min-h-[160px]">
            ...
            {status === "loading" && (
              <div className="flex-1 flex flex-col items-center justify-center gap-3 py-6">
                <Icons.spinner className="size-7 text-warning animate-spin" />
                <p className="text-sm text-muted-foreground">
                  Fetching and processing corporate events…
                </p>
              </div>
            )}
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <StatCard
                    icon={Icons.edit}
                    label="Updated"
                    value={data.totalUpdated.toLocaleString()}
                    color="text-accent"
        ...
        <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            >
              <Icons.refresh className={`size-3.5 ${logsLoading ? "animate-spin" : ""}`} />
              Refresh
            </Button>
          </div>
          ...
          {logsLoading && logs.length === 0 && (
            <div className="flex flex-col items-center justify-center gap-3 py-12">
              <Icons.spinner className="size-6 text-warning animate-spin" />
...
// ── Daily Refresh Card (job-based) ─────────────────────────────────────────
const DailyRefreshCard = () => {
  const [status, setStatus] = useState<Status>("idle");
  const [result, setResult] = useState<ApiResponse | null>(null);
  ...
  const handleTrigger = async () => {
    ...
    try {
      ...
      if (json.success) {
        const d = json.data;
        toast.success("Daily event refresh complete", {
          description: d
            ? `${d.totalInserted.toLocaleString()} inserted · ${d.totalUpdated.toLocaleString()} updated · ${d.totalSkipped.toLocaleString()} skipped`
            : "Upcoming corporate events refreshed.",
        });
      } else {
        toast.error("Daily event refresh failed", {
          description:
            json.error ?? json.message ?? "The refresh did not complete.",
        });
      }
    } catch {
      setResult({ success: false, error: "Network error — server unreachable." });
      setStatus("error");
      toast.error("Couldn't reach the backend — nothing was saved.", {
        description: "Network error — server unreachable.",
      });
    ...
    <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-5">
      <div>
        <div className="flex items-center gap-2 mb-1">
          <Icons.calendar weight="duotone" className="size-4 text-warning" />
          <h2 className="font-semibold text-base">Daily Event Refresh</h2>
        </div>
        <p className="text-xs text-muted-foreground leading-relaxed">
          Fetches the next 30 days of NSE corporate events and upserts new or
          updated records into the database.
        </p>
      </div>

      <div className="flex flex-wrap gap-2">
        {["Fetches NSE corporate events", "Upserts changed records", "Safe to run multiple times"].map((label) => (
          <span
            key={label}
            className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-warning/10 border border-warning/20 text-warning"
          >
      ...
      <div className="flex items-center gap-3">
        <Button
          ...
          disabled={status === "loading"}
          size="sm"
          className="gap-2 bg-warning hover:bg-warning active:bg-warning text-foreground font-semibold px-5 py-2.5 rounded-lg shadow-md shadow-warning/30 hover:shadow-lg hover:shadow-warning/40 transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {status === "loading" ? (
            <><Icons.spinner className="size-4 animate-spin" />Running…</>
          ) : (
            <><Icons.play className="size-4" />Trigger Daily Refresh</>
          )}
        </Button>
        {status !== "idle" && status !== "loading" && (
      ...
      {status === "loading" && (
        <div className="flex items-center gap-2.5 p-3.5 rounded-lg bg-muted/30 border border-border/50">
          <Icons.spinner className="size-4 text-warning animate-spin shrink-0" />
          <p className="text-xs text-muted-foreground">Fetching and processing corporate events…</p>
        </div>
      )}
      ...
      {status === "error" && (
        <div className="flex items-center gap-2.5 p-3.5 rounded-lg bg-destructive/10 border border-destructive/20">
          <Icons.warning className="size-4 text-destructive shrink-0" />
          <div>
            <p className="text-xs font-medium text-destructive">Refresh Failed</p>
            <p className="text-xs text-destructive/80 mt-0.5">
              {result?.error ?? result?.message ?? "An unexpected error occurred."}
            </p>
          </div>
      ...
      {status === "success" && data && (
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-2.5 p-3.5 rounded-lg bg-success/10 border border-success/20">
            ...
            <p className="text-xs font-medium text-success">Refresh completed successfully</p>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <StatCard icon={Icons.calendar} label="From" value={formatDate(data.fromDate)} color="text-warning" bg="bg-warning/10 border-warning/20" />
            <StatCard icon={Icons.calendar} label="To" value={formatDate(data.toDate)} color="text-warning" bg="bg-warning/10 border-warning/20" />
            <StatCard icon={Icons.timer} label="Duration" value={formatDuration(data.durationMs)} color="text-warning" bg="bg-warning/10 border-warning/20" />
            <StatCard icon={Icons.download} label="Fetched" value={data.totalFetched.toLocaleString()} color="text-primary" bg="bg-primary/10 border-primary/20" />
            <StatCard icon={Icons.database} label="Inserted" value={data.totalInserted.toLocaleString()} color="text-success" bg="bg-success/10 border-success/20" />
            <StatCard icon={Icons.edit} label="Updated" value={data.totalUpdated.toLocaleString()} color="text-accent" bg="bg-accent/10 border-accent/20" />
          </div>
          <div className="flex items-center justify-between px-4 py-3 rounded-lg border border-border/50 bg-muted/30">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <Icons.skipForward className="size-4" />
            ...
            <span className="text-sm font-semibold tabular-nums">{data.totalSkipped.toLocaleString()}</span>
          </div>
        </div>
      )}
...
const jobMeta: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  pending:   { label: "Pending",   color: "text-warning",          icon: <Icons.clock className="size-4 text-warning" /> },
  running:   { label: "Running",   color: "text-primary",          icon: <Icons.spinner className="size-4 text-primary animate-spin" /> },
  succeeded: { label: "Succeeded", color: "text-success",          icon: <Icons.success className="size-4 text-success" /> },
  failed:    { label: "Failed",    color: "text-destructive",      icon: <Icons.xCircle className="size-4 text-destructive" /> },
  cancelled: { label: "Cancelled", color: "text-muted-foreground", icon: <Icons.prohibit className="size-4 text-muted-foreground" /> },
};
...
const BackfillSection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await adminFetch(`${API_BASE}/admin/events/backfill`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="w-full border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-6">
      <div>
        <div className="flex items-center gap-2 mb-1">
          <Icons.history weight="duotone" className="size-4 text-warning" />
          <h2 className="font-semibold text-base">Historical Backfill</h2>
          <span className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-warning/10 text-warning border border-warning/20">
            Job-based
          </span>
        </div>
        <p className="text-xs text-muted-foreground">
          Fetch corporate event records for the past N days and upsert them into the database as a background job.
          Defaults to 365 days if no value is provided.
        </p>
      </div>
        ...
        <div className="flex items-end gap-3">
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              placeholder="365"
              disabled={isRunning}
              className="w-32 rounded-lg border border-border/60 bg-background/80 px-3 py-2 text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-warning/40 transition disabled:opacity-50"
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            {jobStatus === "done" || jobStatus === "error" ? (
              ...
              <Button
                type="submit"
                disabled={isRunning}
                className="gap-2 bg-warning hover:bg-warning text-foreground disabled:opacity-60"
              >
                {isRunning ? (
                  <><Icons.spinner className="size-4 animate-spin" />{jobStatus === "submitting" ? "Submitting…" : "Processing…"}</>
                ) : (
                  <><Icons.play className="size-4" />Start Backfill</>
                )}
              </Button>
            )}
            ...
            {canCancel && (
              ...
              >
                {cancelLoading ? <Icons.spinner className="size-3.5 animate-spin" /> : <Icons.prohibit className="size-3.5" />}
                Cancel Job
              </Button>
            )}
    ...
    <div className="flex flex-col gap-2 p-3.5 rounded-lg border border-border/50 bg-muted/20 hover:bg-muted/40 transition-colors duration-150">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs font-semibold capitalize ${isSuccess ? "text-success" : "text-destructive"}`}>
            {log.status}
          </span>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <LogPill label="From" value={formatDate(log.fromDate)} />
        <LogPill label="To" value={formatDate(log.toDate)} />
        ...
        <LogPill label="Updated" value={log.totalUpdated.toLocaleString()} />
...
const LogPill = ({ label, value, accent }: { label: string; value: string; accent?: boolean }) => (
  <div className="flex items-center gap-1">
    ...
    <span className={`text-[11px] font-semibold tabular-nums ${accent ? "text-success" : "text-foreground/80"}`}>
      {value}
    </span>
  </div>
);
