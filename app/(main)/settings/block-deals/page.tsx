...
const BlockDealsPage = () => {
  ...
  const handleTrigger = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/admin/deals/trigger`, {
        method: "POST",
          ...
          <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-6">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <span
                  key={label}
                  className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-violet-500/10 border border-violet-500/20 text-violet-400"
                >
                  {label}
                </span>
              ))}
            ...
            <div className="flex items-center gap-3">
              <Button
                ...
                size="lg"
                className="gap-2.5 bg-violet-600 hover:bg-violet-700 active:bg-violet-800 text-white font-semibold px-6 py-3 rounded-lg shadow-md shadow-violet-900/30 hover:shadow-lg hover:shadow-violet-900/40 transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {status === "loading" ? (
                  <>
...
const BackfillSection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/admin/deals/backfill`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        <div className="flex items-end gap-3">
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              placeholder="90"
              disabled={isRunning}
              className="w-32 rounded-lg border border-border/60 bg-background/80 px-3 py-2 text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-violet-500/40 transition disabled:opacity-50"
            />
          </div>
