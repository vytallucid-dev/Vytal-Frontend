...
const CorporateEventsPage = () => {
  ...
  const handleTrigger = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/admin/events/refresh`, {
        method: "POST",
          ...
          <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-6">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <span
                  key={label}
                  className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-400"
                >
                  {label}
                </span>
              ))}
            ...
            <div className="flex items-center gap-3">
              <Button
                ...
                size="lg"
                className="gap-2.5 bg-orange-600 hover:bg-orange-700 active:bg-orange-800 text-white font-semibold px-6 py-3 rounded-lg shadow-md shadow-orange-900/30 hover:shadow-lg hover:shadow-orange-900/40 transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {status === "loading" ? (
                  <>
    ...
    <div className="border rounded-xl border-border/50 bg-background/50 backdrop-blur-sm shadow-sm p-6 flex flex-col gap-5">
      ...
      <div className="flex items-center gap-3">
        <Button
          ...
          disabled={status === "loading"}
          size="sm"
          className="gap-2 bg-orange-600 hover:bg-orange-700 active:bg-orange-800 text-white font-semibold px-5 py-2.5 rounded-lg shadow-md shadow-orange-900/30 hover:shadow-lg hover:shadow-orange-900/40 transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {status === "loading" ? (
            <><Loader2 className="w-4 h-4 animate-spin" />Running…</>
          ) : (
            <><Play className="w-4 h-4" />Trigger Daily Refresh</>
          )}
        </Button>
        {status !== "idle" && status !== "loading" && (
      ...
      {status === "success" && data && (
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-2.5 p-3.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
            ...
            <p className="text-xs font-medium text-emerald-400">Refresh completed successfully</p>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <StatCard icon={Calendar} label="From" value={formatDate(data.fromDate)} color="text-orange-400" bg="bg-orange-500/10 border-orange-500/20" />
            <StatCard icon={Calendar} label="To" value={formatDate(data.toDate)} color="text-orange-400" bg="bg-orange-500/10 border-orange-500/20" />
            <StatCard icon={Timer} label="Duration" value={formatDuration(data.durationMs)} color="text-amber-400" bg="bg-amber-500/10 border-amber-500/20" />
            <StatCard icon={Download} label="Fetched" value={data.totalFetched.toLocaleString()} color="text-sky-400" bg="bg-sky-500/10 border-sky-500/20" />
            <StatCard icon={Database} label="Inserted" value={data.totalInserted.toLocaleString()} color="text-emerald-400" bg="bg-emerald-500/10 border-emerald-500/20" />
            <StatCard icon={PenLine} label="Updated" value={data.totalUpdated.toLocaleString()} color="text-violet-400" bg="bg-violet-500/10 border-violet-500/20" />
          </div>
          <div className="flex items-center justify-between px-4 py-3 rounded-lg border border-border/50 bg-muted/30">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <SkipForward className="w-4 h-4" />
        ...
        <div className="flex items-end gap-3">
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              placeholder="365"
              disabled={isRunning}
              className="w-32 rounded-lg border border-border/60 bg-background/80 px-3 py-2 text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-orange-500/40 transition disabled:opacity-50"
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            {jobStatus === "done" || jobStatus === "error" ? (
              ...
              <Button
                type="submit"
                disabled={isRunning}
                className="gap-2 bg-orange-600 hover:bg-orange-700 text-white disabled:opacity-60"
              >
                {isRunning ? (
