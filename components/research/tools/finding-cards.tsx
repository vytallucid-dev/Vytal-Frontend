...
export function FindingCards({
  ...
  if (!findings.length && !ended?.length && !notCovered?.length && !crossTool?.count && !showQuiet) return null;
  return (
    <Panel className="px-4 py-4">
      ...
      {!!crossTool?.count && <CrossToolLine s={crossTool} symbol={symbol ?? null} />}
...
/**
 * ★ TESTED AND NOT SHIPPED — plainly separate from findings, and deliberately the quietest thing on
 * the page.
 *
