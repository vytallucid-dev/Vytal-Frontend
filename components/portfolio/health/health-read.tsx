...
import type { Holding, LensProfile, PillarProfile, PortfolioSnapshot } from "@/types/portfolio";
import type { PortfolioTab } from "../tabs";
import { DRAG_COLOR, POSITIVE_COLOR, healthColor, healthPatternFindings } from "../lib";
import {
  SIGNAL_SOURCE_META,
  activeSignals,
  type DeductionStory,
...
function Waterfall({ b, color }: { b: DeductionStory; color: string }) {
  ...
  type Col = { key: string; label: string; sub: string; value: number; color: string; top: number; bottom: number; drag: boolean; calm?: boolean };
  const cols: Col[] = [
    { key: "anchor", label: `${r0(q)}`, sub: "quality · anchor", value: q, color: FOUNDATION, top: q, bottom: 0, drag: false },
    { key: "flags", label: calm ? "0" : `−${r0(fD)}`, sub: calm ? "no active flags" : "active flags", value: fD, color: DRAG_COLOR.signal, top: q, bottom: q - fD, drag: true, calm },
    { key: "health", label: `${health}`, sub: "= health", value: health, color, top: health, bottom: 0, drag: false },
  ];
  const M = cols.length;
    ...
    <div className="rounded-xl border border-line bg-surface-2 p-4 sm:p-5">
      <div className="relative" style={{ height: H }}>
        ...
        <svg viewBox={`0 0 100 ${H}`} preserveAspectRatio="none" className="absolute inset-0 h-full w-full" aria-hidden>
          <path d={path} fill="none" stroke="var(--ink3)" strokeWidth={1} strokeDasharray="2.5 3" opacity={0.6} vectorEffect="non-scaling-stroke" />
        </svg>
        {/* columns */}
        <div className="absolute inset-0 flex items-end">
          {cols.map((c) => {
            const hgt = Math.max((c.value / 100) * H, c.drag ? (c.calm ? 0 : 3) : 3);
            const bottomPx = (c.bottom / 100) * H;
            const anchorEdge = !c.drag;
            return (
              <div key={c.key} className="relative flex-1" style={{ height: H }}>
                {c.calm ? (
                  <div className="absolute left-1/2 -translate-x-1/2" style={{ bottom: bottomPx }}>
                    <Icons.check weight="bold" className="size-3.5" style={{ color: POSITIVE_COLOR }} />
                  </div>
                ) : (
                  <div
                    ...
                    style={{
                      bottom: bottomPx,
                      height: hgt,
                      background: c.drag ? c.color : `color-mix(in oklch, ${c.color} 35%, transparent)`,
                      borderTop: anchorEdge ? `2px solid ${c.color}` : undefined,
                    }}
                  />
                )}
              </div>
      ...
      <div className="mt-2 flex items-start">
        {cols.map((c) => (
          <div key={c.key} className="flex flex-1 flex-col items-center text-center">
            <span className="num text-[16px] font-semibold leading-none" style={{ color: c.calm ? POSITIVE_COLOR : c.color }}>
              {c.label}
            </span>
            <span className="mt-1 text-[8.5px] uppercase leading-tight tracking-wide text-ink3">{c.sub}</span>
          </div>
        ))}
      </div>

      {/* the proportional split — health vs the flags drag */}
      <div className="mt-3.5 flex h-2 gap-0.5 overflow-hidden rounded-full">
      ...
      <p className="mt-3 text-[11.5px] leading-relaxed text-ink3">
        Your businesses are <span className="num text-ink2">{r0(q)}</span>-healthy;{" "}
        {calm ? (
          <>no active red flags subtract anything</>
        ) : (
          <>active red flags subtract <span className="num" style={{ color: DRAG_COLOR.signal }}>{r0(fD)}</span></>
        )}{" "}
        — landing at <span className="num" style={{ color }}>{health}</span>. Nothing about how you&apos;ve built the book
        touches this number — that&apos;s the Construction read.
      </p>
    </div>
  );
}
...
const SOFT_SPOT: Record<PillarKey, string> = {
  foundation: "the balance-sheet and returns base is the softer spot — durable, not the driver here",
  momentum: "Momentum is the soft spot — sound businesses, not currently accelerating",
  market: "the market read is the softer spot — steady names the market isn't chasing",
...
function characterLine(pp: PillarProfile): string {
  ...
  if (strong.v - weak.v < 6) return "Evenly balanced across all four pillars — no single source carries the book's health.";
  ...
  return (
    <div className="rounded-xl border border-line bg-surface-2 p-4 sm:p-5">
      <div className="mb-4 flex items-baseline justify-between">
        <p className="kicker">Where your book&apos;s health comes from</p>
        <p className="text-[10px] text-ink3">position-weighted across scored holdings</p>
      </div>
        ...
        {PILLAR_ORDER.map((k) => {
          ...
          return (
            <div key={k} className="flex items-center gap-3">
              <span className="w-[92px] shrink-0 text-[12px] text-ink2">{meta.label}</span>
              <div className="relative h-3 flex-1 overflow-hidden rounded-full bg-surface-3">
                <div className="h-full rounded-full" style={{ width: `${Math.max(v, 1)}%`, background: meta.cssVar }} />
              </div>
              <span className="num w-8 shrink-0 text-right text-[14px] font-semibold" style={{ color: meta.cssVar }}>
                {r0(v)}
              </span>
            </div>
          );
      ...
      <p className="mt-4 border-t border-line pt-3 text-[12px] leading-relaxed text-ink2">{characterLine(pp)}</p>
    </div>
  );
}
...
const LENS_META: { key: keyof NonNullable<LensProfile>; label: string; color: string }[] = [
  ...
  return (
    <div className="rounded-xl border border-line bg-surface-2 p-4 sm:p-5">
      <div className="mb-3 flex items-center gap-2">
        <p className="kicker">The character of that health</p>
        <span className="rounded-full border border-line2 bg-surface-3 px-2 py-0.5 text-[10px] text-ink3">based on fired patterns</span>
      </div>
      <div className="flex h-2.5 w-full gap-0.5 overflow-hidden rounded-full">
        {segs.map((m) => (
          <div key={m.key} style={{ flex: Math.max(lp[m.key], 0.001), background: m.color }} title={`${m.label} ${pct1(lp[m.key])}`} />
        ))}
      </div>
      <div className="mt-2.5 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-ink2">
        {segs.map((m) => (
          <span key={m.key} className="inline-flex items-center gap-1.5">
            <span className="size-2 rounded-[2px]" style={{ background: m.color }} />
            {m.label} <span className="num text-ink3">{pct1(lp[m.key])}</span>
          </span>
        ))}
      </div>
      <p className="mt-3 text-[12px] leading-relaxed text-ink2">{lensCharacterLine(lp)}</p>
      <p className="mt-1 text-[10.5px] leading-relaxed text-ink3">
        This describes the nature of the patterns behind your health — not a share of the score.
      </p>
    </div>
  );
}
...
function SignalsLedger({ snapshot, b }: { snapshot: PortfolioSnapshot; b: DeductionStory }) {
  const active = activeSignals(snapshot);
  const calm = active.length === 0;
  return (
    <div className="rounded-xl border border-line bg-surface-2 p-4 sm:p-5">
      <div className="mb-1 flex items-center gap-2">
        <span className="grid size-6 place-items-center rounded-md" style={{ background: `color-mix(in oklch, ${DRAG_COLOR.signal} 14%, transparent)`, color: DRAG_COLOR.signal }}>
      ...
      <p className="mb-3.5 text-[11px] leading-relaxed text-ink3">
        {calm ? (
          <>No holding is firing a red flag — Signals took <span className="text-ink2">nothing</span> off.</>
        ) : (
          <>Each flagged holding deducts by its book weight — <span className="num" style={{ color: DRAG_COLOR.signal }}>−{r0(b.flagsDrag)}</span> off the Health Score in total.</>
        )}
      </p>
      {calm ? (
          ...
          {active.map((e) => {
            ...
            return (
              <div key={e.symbol} className="flex items-center gap-3 rounded-md border border-line bg-surface-1 px-3 py-2">
                <span className="num shrink-0 rounded bg-surface-3 px-1.5 py-0.5 text-[11px] font-semibold text-ink">{e.symbol}</span>
                ...
                <span className="num shrink-0 text-[14px] font-semibold" style={{ color: DRAG_COLOR.signal }}>
