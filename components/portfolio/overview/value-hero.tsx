...
export function ValueHero({ holdings, totals }: { holdings: Holding[]; totals: HoldingsTotals }) {
  ...
  const sectorCount = new Set(holdings.map((h) => h.sector ?? "Unclassified")).size;
  ...
  const kpis: { label: string; value: string; color?: string }[] = [
    ...
    { label: "Holdings", value: String(totals.positions) },
    ...
    <div className="overflow-hidden rounded-xl border border-line bg-surface-1 p-5 sm:p-6">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="min-w-0">
          ...
          <AnimatedNumber
            ...
            prefix="₹"
            locales="en-IN"
            className="block text-[44px] font-semibold leading-none text-ink sm:text-[52px]"
          />
          ...
          {unpriced > 0 && (
            <p className="mt-2 text-[11.5px] text-ink3">
              {unpriced} of {totals.positions} holdings have no live price yet — value &amp; return reflect the {totals.pricedPositions} priced.
            </p>
          )}
        </div>
      ...
      <div className="mt-6 border-t border-line pt-5">
        {navQ.isLoading ? (
          ...
          <NavChart series={navQ.data.series} />
        ) : (
          <>
            <StubHeader />
